...
function variableName()  //para guardar la variable name
{
    ...
    textoName.value = "";//limpiamos contenido.
...
function start()
{
    ...
    intervalHandler = setInterval(updateTemp, 1000); // Inicializamos temporizador
...
{ 
    ...
    var newArr = newRosco.questions.filter((question)=> {return question.status !== 0 && question.status !== 3});
    ...
    //entran las que han sido contestadas con "pasapalabra".    
    if (newRosco.questions[indiceLetraActual].status === 3) 
    {
        pregunta = letra.pasapalabra;//valor que le asigno al contestar "pasapalabra"
    ...
    else if (newRosco.questions[indiceLetraActual].status === 0)
    {
...
function inputAnswer(respuesta)
{
    var letra = newRosco.questions[indiceLetraActual];
    if (respuesta.toLowerCase() === "pasapalabra")
    {   if (newRosco.questions[indiceLetraActual].status === 3)
        {   
            //si se elige pasapalabra por 2ª vez.(indiceOpcionLetraActual no lo queremos)
            pasapalabra = newRosco.questions[indiceLetraActual].pasapalabra;
        }
        else
        {   
            //guardamos la pregunta en una variable pasapalabra
            pasapalabra = newRosco.questions[indiceLetraActual].question[indiceOpcionLetraActual] ;
            newRosco.questions[indiceLetraActual].pasapalabra = pasapalabra;
            newRosco.questions[indiceLetraActual].status = 3; 
        }
...
function GameOver()
{
    finalStyle();//cambio de estilo de pantalla y font-size final
...
function upDate()//reloj de hora actual y fecha.
{
...
function deactivateTemp()//para parar temporizador
{
...
pantRespuesta.addEventListener("keypress",keyPress);
...
function keyPress(e)
{
    ...
    {
        outputAnswer(); //función que despues de enter ejecutará el código:
    }
}
function outputAnswer()
{
    var contenidoRespuesta = pantRespuesta.value; //atributo "value" por ser un input.
    inputAnswer(contenidoRespuesta);
...
function ranking()
{
    var newRanking = new ObjectRanking(contenidoName,aciertos); //instanciamos objero ranking
...
botonVolver.addEventListener("click",playAgain);
botonVolver2.addEventListener("click",out);

function playAgain()//volvemos al inicio
{
    var pantalla4 = document.getElementById("pantalla4");

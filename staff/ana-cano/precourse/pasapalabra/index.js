...
// FUNCTIONS
function preguntasPasapalabra (){
    for( let i = 0; i<rosco.length; i++){
        currentQuestion = prompt(rosco[i].question);
        if (currentQuestion == rosco[i].answer){
            alert('Correcto')
            acertadas++;
        } else if (currentQuestion === 'PASAPALABRA'){
            rosco[i].status=1;
        } else {
            alert('Incorrecto')
            rosco[i].status=2;
...
function Player(name, points) {
    this.name = nameGamer;
    this.color = acertadas;
  }

function preguntasPendientes(){
    for( let i = 0; i<rosco.length; i++){
    if (rosco[i].status === 1) {
    prompt(rosco[i].question)
    }
}
}
...
if(playAgain === true){
jugarPasapalabra();
} else { alert(`Gracias por haber jugado con nosotros`);
}
}

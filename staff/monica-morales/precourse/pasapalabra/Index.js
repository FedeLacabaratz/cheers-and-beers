...
function question(){
    for(i in questions){
        var pregunta=prompt(questions[i].question).toLocaleLowerCase();
        if(pregunta===''){
            alert('Por favor, introduzca una respuesta o PASAPALABRA.');
            pregunta = prompt(questions[i].question).toLocaleLowerCase();
        }if(pregunta===questions[i].answer){
            questions[i].status='OK';
            aciertos++;
         }else if(pregunta==='pasapalabra'){
            questions[i].status='PASAPALABRA';
         }else if(pregunta!=questions[i].answer){
            questions[i].status='WRONG';
            alert(`Ohhhh, la palabra correcta es: ${(questions[i].answer).toUpperCase()}.`);
            fallos++;
        }
    }
};
...
function nextRound(){
    for(i in questions){
        if(questions[i].status==='PASAPALABRA'){
            var pregunta=prompt(questions[i].question).toLocaleLowerCase();
        if(pregunta===''){
            alert('Por favor, introduzca una respuesta o PASAPALABRA.');
            pregunta = prompt(questions[i].question).toLocaleLowerCase();
    ...
    var respuestas=fallos+aciertos;
    if(respuestas!=27){
        nextRound();
    }
};
...
function gamePasapalabra(){
    ...
    nextRound();
    ranking();
    var newGame=confirm(`${name},¿quieres volver a jugar?`);
    if(newGame===true){
        gamePasapalabra();  
    }else{
